...
http.createServer((req, res) => {
  res.writeHead(200);
  res.end('DigitaAí online!');
}).listen(process.env.PORT || 3000, () => {
  console.log('✅ Servidor HTTP ativo — pronto para UptimeRobot');
});
...
async function startRound(channel, gs) {
  ...
  const slowTimer = setTimeout(async () => {
    ...
    for (const [, player] of gs.players) {
      if (player.lives > 0 && !gs.roundAnswered.has(player.userId)) {
  ...
  collector.on('collect', async msg => {
    ...
    if (allAnswered) { clearTimeout(slowTimer); collector.stop('all_answered'); }
  });
