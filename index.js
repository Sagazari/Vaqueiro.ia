...
function formatLives(lives) { return '❤️'.repeat(Math.max(0,lives)) + '🖤'.repeat(Math.max(0, 3 - lives)); }
...
async function startRound(channel, gs) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: 'Digite a palavra EXATAMENTE como aparece! ⚠️ +20s = -2 corações!' })
    .setTimestamp();

  await channel.send({ embeds: [embed] });

  // Timer de 20s — quem não respondeu perde 2 corações
  const slowTimer = setTimeout(async () => {
    ...
    for (const [id, player] of gs.players) {
      if (player.lives > 0 && !gs.roundAnswered.has(id)) {
        ...
        if (player.lives <= 0) {
          await channel.send(`💀 **${player.username}** demorou demais e foi **eliminado!**`);
        } else {
          await channel.send(`⚠️ **${player.username}** demorou demais! -2 ❤️ ${formatLives(player.lives)}`);
        }
      }
    }
  }, 20000);
  ...
  collector.on('collect', async msg => {
    ...
    // Verifica se todos os vivos já responderam — avança imediatamente
    const aliveNow = [...gs.players.values()].filter(p => p.lives > 0);
    const allAnswered = aliveNow.every(p => gs.roundAnswered.has(p.userId));
    if (allAnswered) {
  ...
  collector.on('end', async (_, reason) => {
    ...
    const loadMsg = await channel.send('⏳ **Carregando próxima palavra...**');
